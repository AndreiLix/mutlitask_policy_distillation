...
class AntDirectionTaskWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        ######### chat gpt walking in a cirle ################

          # calculate reward based on circular trajectory
        pos = self.get_body_com("torso")
        if self.t == 0:
            self.prev_pos = pos

        # calculate angle between current and previous positions
        x1, y1 = pos[:2]
        x2, y2 = self.prev_pos[:2]
        angle = np.arctan2(y1, x1) - np.arctan2(y2, x2)
        angle = np.abs(((angle + np.pi) % (2 * np.pi)) - np.pi)  # wrap to [-pi, pi]
        Z
        # calculate desired angle based on fixed point on circle
        radius = 0.5  # adjust as needed
        center = np.array([0, 0])
        diff = pos[:2] - center
        desired_angle = np.arctan2(diff[1], diff[0])
        
        # calculate reward based on difference between angles
        reward = np.cos(4*(angle - desired_angle))  # range [-1, 1]
        self.prev_pos = pos
...
if __name__=="__main__":
    ...
    model = PPO("MultiInputPolicy", env, policy_kwargs=policy_kwargs, verbose=1, n_steps=1024, batch_size=256, n_epochs=5, gamma=0.99, tensorboard_log="tensorboard/")
    ...
    model.save("checkpoints/ant_base")
